  ...
  async save() {
    ...
    if (this.state.datos.name !== "") {
      if (this.state.datos.electiveMonths !== "") {
        if (
          this.state.datos.electiveMonths > -1 &&
          this.state.datos.electiveMonths < 51
        ) {
          if (this.state.datos.practiceMonths !== "") {
            if (
              this.state.datos.practiceMonths > -1 &&
              this.state.datos.practiceMonths < 51
            ) {
              try {
                let datos = await Api(
              ...
              agregarError(
                document.getElementById("practiceMonths"),
            ...
            agregarError(
              document.getElementById("practiceMonths"),
              "Debes llenar este campo o ingresar números"
            );
          }
        } else {
          agregarError(
            document.getElementById("electiveMonths"),
            "Debes poner un número entre 0 y 50"
          );
        }
      } else {
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="crearLabel">
                Crear tipo de programa de formación
              </h5>
              <button
                type="button"
                className="close"
